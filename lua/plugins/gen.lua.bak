...
return {
  ...
  config = function()
    require("gen").prompts["Add docstring"] = {
      prompt = "Add docstring in javascript format to the following function: $register. Only ouput the result in format ```$filetype\n...\n```:\n```$filetype\n$text\n```",
    }
    require("gen").prompts["Add test"] = {
      prompt = "Add tests using vitest for the following function: $register. Only ouput the result in format ```$filetype\n...\n```:\n```$filetype\n$text\n```",
      extract = "```$filetype\n(.-)```",
    }
    ...
    vim.keymap.set("n", "<leader>ad", "<cmd>Gen Add docstring<CR>", { desc = "Add docstring to function" })
    vim.keymap.set(
      "v",
      "<leader>ac",
    ...
    vim.keymap.set("n", "<leader>at", "<cmd>Gen Add test<CR>", { desc = "Add test" })
  end,
}
